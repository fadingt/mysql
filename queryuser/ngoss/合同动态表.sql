...
from (select
	case 
		when ttt.ponotype = 1 then 0 
		when ttt.type in (2,4) and SUBSTR(ttt.filetime FROM 1 FOR 4)=SUBSTR(@yearmonth FROM 1 FOR 4) then ifnull(ttt.contractprice,0) 
		when ttt.type in (1,3) and SUBSTR(ttt.ddtime FROM 1 FOR 4)=SUBSTR(@yearmonth FROM 1 FOR 4) then ifnull(ttt.contractprice,0)
		else 0 
	end as jnhtje,
	case 
		when ttt.ponotype = 1 then 0 
		when ttt.type in (2,4) and SUBSTR(ttt.filetime FROM 1 FOR 6)=SUBSTR(@yearmonth FROM 1 FOR 6) then ifnull(ttt.contractprice,0)
		when ttt.type in (1,3) and SUBSTR(ttt.ddtime FROM 1 FOR 6)=SUBSTR(@yearmonth FROM 1 FOR 6) then ifnull(ttt.contractprice,0) 
		else 0 
	end as dyhtje,
ttt.orderprice2 as yorderprice,
...
from (select (case when c2.effectstatus=5  then '是'  else '否' end ) effectstatus ,c2.contractid,c2.billyjc,  c2.contractno,c2.contractname,c2.pono,c2.ponotype as beiyong,c2.filetime,c2.orderprice2, c2.ponotype,
...
getcustname(c2.firstparty)firstparty,c2.secondparty,
-- finalcustomer c2.finalcust as company,
cust.custname as company, getusername(cust.tecpersonid) as tecperson,
case when c2.type in (2,4) then ifnull(c2.contractprice,0) when c2.type in (1,3) and c2.ponotype = 2 then ifnull(c2.poprice,0) end as contractprice,
...
a8.receyulate,t1.billt1,t1.billt2,t1.billt3,t1.billt4,t1.billt5,t1.billt6,t2.recet1,t2.recet2,t2.recet3,t2.recet4,t2.recet5,t2.recet6 from(select 
 cc.lastmodtime1 as ddtime,d1.id,getBillYuJingCout(cc.contractid,0) as billyjc,d1.pono,d1.extend3 as ponotype,d1.orderprice2, d1.poprice,cc.* from  (select c1.*, ctemp.lastmodtime1 from t_contract_main c1
left join(
	SELECT 
		contractid contractid1, lastmodtime lastmodtime1 
	from t_contract_maintmp a, (SELECT MIN(id) id1 from t_contract_maintmp GROUP BY contractid) b 
	WHERE a.id = b.id1 and status = 3 and type in (2,4)
)ctemp on ctemp.contractid1 = c1.contractid
where c1.effectstatus not in ('7','8') and c1.type in (2,4) group by c1.contractid )cc
left join ( select sum(poprice) poprice,id,pono,protocolorcontractid,extend3,SUM(case when extend3 = 1 then poprice else 0 end) orderprice2 from   t_contract_order  where left(createtime, 4) = SUBSTR(@yearmonth FROM 1 FOR 4)  GROUP BY protocolorcontractid  )d1 on cc.contractid=d1.protocolorcontractid)c 
...
b7_2017.receshiyear17,b8.receyulate,t1.billt1,t1.billt2,t1.billt3,t1.billt4,t1.billt5,t1.billt6,t2.recet1,t2.recet2,t2.recet3,t2.recet4,t2.recet5,t2.recet6 from (select d1.createtime as ddtime,d1.id,getBillYuJingCout(d1.id,1) as billyjc,d1.pono,d1.extend3 as ponotype,d1.orderprice2,d1.poprice,cc.* from (select c1.*, null from t_contract_main c1 
where c1.effectstatus not in ('7','8')  and c1.type in (1,3)group by c1.contractid)cc join (SELECT d1.*, (case when d1.extend3 =1 then poprice else 0 end) as orderprice2 from t_contract_order d1)d1 on cc.contractid=d1.protocolorcontractid) c left join (select contractid, pono, sum(j.poprice) as billyunow, billyutime from view_orderstage_bill j where billyutime=date_format(@yearmonth,'%Y%m') group by pono)b1 on c.contractid=b1.contractid and c.pono=b1.pono left join (select sum(f.invoiceamt) as billshinow,s.protocolorcontractid as contractid,d.pono from (select * from t_bill_Invoice  where invoicestatus!=1) f join (select * from t_bill_main where invoicestatus=3) s on f.billingid=s.id 
