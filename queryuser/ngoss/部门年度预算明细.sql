...
(
	select 
		budgetid, budgetno, budgetname, budgettype, budgetdescription,-- 预算编号 名称 类型 描述
...
left join (
	...
	from (  
		...
		( 
				...
				where workhourtype=2 
				and status = 3 
				GROUP BY employeeid,targetid,left(yearmonthday,6)
				UNION ALL 
				select FORMAT(sum(workhours)/8,2) as workdays,employeeid,targetid,left(yearmonthday,6) yearmonth from t_project_workhourmanager  
				where workhourtype=2 
				and status = 2 
				GROUP BY employeeid,targetid,left(yearmonthday,6)
