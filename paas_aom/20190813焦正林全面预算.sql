...
null `预计人天`
				...
				left join mdl_aos_hrorg b1 on left(A.S_YSORGCODE, 10) = b1.s_orgcode
				left join mdl_aos_hrorg c1 on left(A.S_YSORGCODE, 13) = c1.s_orgcode
				left join mdl_aos_hrorg d1 on left(A.S_YSORGCODE, 16) = d1.s_orgcode
			where a.IS_DELETE = 0
)base
left join (
		SELECT
