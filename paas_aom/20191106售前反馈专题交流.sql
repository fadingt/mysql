...
left join (
	SELECT 
		ID, I_PRESALEID, CREATE_TIME `反馈时间`, S_PSFBCODE `售前反馈编码`,
		S_FBSTATUS	`反馈状态`, S_APPSTATUS	`审批状态`,
		S_LEADERS	`参会主要领导`, S_MONTHS	`预计项目周期`, DT_BMONTH	`预计项目启动月份`, DL_AMOUNT	`预计项目金额`,
		S_TARGET	`行方目前设定目标`,S_REASON	`行方启动项目原因`,S_COMATTDES `附件说明`,
		null `竞争对手`
