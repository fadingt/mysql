...
join (
	...
	where year(DT_CANDAY) = 2019
)t
left join (
		SELECT
			budgetno, budgettype, yearmonth,
			SUM(case when type = 1 then amt else 0 end) '标准工资',
			SUM(case when type = 2 then amt else 0 end) '标准十三薪奖金',
			SUM(case when type = 3 then amt else 0 end) '差旅费',
			SUM(case when type = 4 then amt else 0 end) '实际工资',
...
)rlcb on rlcb.budgetno = base.budgetno and rlcb.yearmonth = t.yearmonth
left join (
		SELECT  budgetno, SUM(debit) sjfy, yearmonth
		from t_snap_fi_voucher
		where 1=1
		and yearmonth is not null and dc = '01'
		and budgetno is not null and budgetno<>12 and budgetno<>''
		GROUP BY budgetno, yearmonth
)fycb on fycb.budgetno = base.budgetno and fycb.yearmonth = t.yearmonth